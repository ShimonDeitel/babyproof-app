import XCTest
@testable import BabyproofChecklist

@MainActor
final class BabyproofChecklistTests: XCTestCase {
    ...
    func test_add_incrementsCount() {
        ...
        _ = store.add(BabyproofChecklistEntry())
    ...
    func test_add_respectsFreeLimit() {
        ...
        let result = store.add(BabyproofChecklistEntry())
    ...
    func test_update_modifiesExistingEntry() {
        let store = Store()
        var entry = BabyproofChecklistEntry()
        _ = store.add(entry)
